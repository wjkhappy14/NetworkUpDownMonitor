...
namespace UpDownMonitor
{
    ...
    public sealed class Deque<T> : IList<T>, IReadOnlyList<T>, System.Collections.IList
    {
        ...
        public Deque(int capacity)
        {
            if (capacity < 0)
            {
                ArgumentOutOfRangeException argumentOutOfRangeException = new ArgumentOutOfRangeException(nameof(capacity), "Capacity may not be negative.");
        ...
        void ICollection<T>.CopyTo(T[] array, int arrayIndex)
        {
            if (array == null)
            {
                ArgumentNullException argumentNullException = new ArgumentNullException(nameof(array));
